...
linespecieslist = ['H2O','CO','CH4','CO2','HCN','C2H2','NH3','SO2']
linefractionlist = [H2Ofraction,COfraction,CH4fraction,CO2fraction,HCNfraction,C2H2fraction,NH3fraction,SO2fraction]
...
axes[1,0].set_title("SImulated emission of V1298 tauri b")
...
axes[1,1].set_title("Simulated T-P profile of V1298 Tauri b")
plt.tight_layout()
plt.show()
...
np.savetxt("/home/michel/code/data/v1298taub_FpFs_photochem1_all.txt",output_data,delimiter=' ')
np.savetxt("/home/michel/code/data/planet_emission.txt",np.array([waverange,CGSFp]),delimiter=' ')
...
X,Y = np.meshgrid(wlen_mu,pressures_bar)
axes.contour(X,Y,other_outputs['emission_contribution'],50,cmap=plt.cm.bone_r)
axes.set_yscale('log')
axes.set_xscale('log')
axes.set_xlabel('Wavelength [micron]')
axes.set_ylabel('Pressure [bar]')
axes.set_title('Emission contribution')
axes.set_ylim([np.max(pressures),np.min(pressures)])
axes.set_xlim([np.min(0.5),np.max(28)])
plt.show()
